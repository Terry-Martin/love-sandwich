...
def get_sales_data():
    ...
    """
    while True:
        print("Please enter sales date from last market.")
...
def validate_data(values):
    ...
    or if there are arent exactly 6 values.
    """
    try:
...
def calulate_suplus_data(sales_row):
    """
...
def get_last_5_entries_sales():
    """
    Collects columns of data from sales worksheet, colling
...
def get_stock_values(data):
    """
    Create dictionary to show how many of each sandwich needs to be made
    """
    headings = SHEET.worksheet("stock").row_values(1)

    # sandwiches_to_make = {}
    sandwiches_to_make = {heading: value
                          for heading, value in zip(headings, data)}

    print(f"We need to make these: {sandwiches_to_make}")
